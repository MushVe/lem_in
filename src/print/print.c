...
/*   Updated: 2020/06/29 09:43:01 by cseguier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	affect_ants(t_combo_data *cd, t_ant_status *ant, int id, int max)
{
	int	id_path;
	...
	{
		if (cd->path_combo[id_path].ants > 0 && id < max)
		{
			cd->path_combo[id_path].room[1].ant = id;
			ant[id].id_path = id_path;
			ant[id].id_room = cd->path_combo[id_path].room[1].id;
