...
/*   Updated: 2020/05/20 19:27:48 by cseguier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		is_the_same(t_path_array *path, int i_current, int i_other)
{
	int	i_room;

	i_room = -1;
	while (++i_room < path[i_current].size)
		if (path[i_current].room[i_room] != path[i_other].room[i_room])
			return (0);
	return (1);
}
...
void	save_path(t_p *p, t_bfs *bfs, int index, int size)
{
	...
	{
		bfs->path_nb++;
		bfs_new_node(bfs->tmp_path, &bfs->path_list, size, 0);
		return ;
	}
	else
	{
...
void	reorder_path(t_bfs *bfs)
{
	...
	bfs->path_array = clean_path_array(bfs->path_array, \
