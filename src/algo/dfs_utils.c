...
/*   Updated: 2020/05/20 19:20:50 by cseguier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		find_best_room(t_room_infos *rooms, int host, int way)
{
	...
	while (i < rooms[host].nb_link)
	{
		...
		if (rooms[link_id].score <= rooms[host].score
			&& is_closer(&d_start, &d_end, way, rooms[link_id]))
			return (link_id);
		i++;
	}
	return (-1);
...
void	get_ultimate_result(t_p *p, t_bfs *bfs)
{
	...
	data = p->data.rooms;
	rooms = bfs->rooms;
	if (rooms[data.end_index].nb_link > rooms[data.start_index].nb_link)
		link = data.start_index;
	else
		link = data.end_index;
	while (++i < p->size)
	{
		...
		{
			if (bfs->rooms[i].token != -1 && (bfs->final_score == 0
